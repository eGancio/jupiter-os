...
pub async fn ensure_qdrant(qdrant_url: &str, data_dir: &Path) -> anyhow::Result<()> {
    ...
    let storage_root = std::env::var("QDRANT_STORAGE_DIR")
        ...
        .unwrap_or_else(|_| {
            let shared = Path::new("../io/data/qdrant");
            if shared.is_dir() {
                shared.to_path_buf()
            } else {
                qdrant_dir.clone()
...
fn extract_zip(archive: &Path, dest: &Path) -> anyhow::Result<()> {
    ...
    for i in 0..zip.len() {
        ...
        // Only extract the binary (skip directories and other files)
        if entry.is_dir() {
            continue;
        }

        // Flatten: extract just the filename, ignore subdirectories in the archive
