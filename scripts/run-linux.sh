...
BIN="./app/src-tauri/target/release/jupiteros"
if [ ! -x "$BIN" ]; then
  echo "ERRORE: binario non trovato: $BIN — esegui prima:  (cd app && npm run tauri build)"
  exit 1
fi
...
# disponibile opt-in:  JUPITEROS_SAFE_GFX=1 scripts/run-linux.sh
