...
#  - Rust workspace (moon-io, moon-europa, jupiteros-shared) in release
#  - Tauri GUI (jupiteros) in release
...
# ---------------------------------------------------------------------------
# 2. Rust workspace (moon-io, moon-europa, jupiteros-shared) — release build
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 3. Tauri GUI (jupiteros)
# ---------------------------------------------------------------------------
build_tauri_gui() {
    log "Installing JS deps for Tauri GUI…"
    pushd jupiteros >/dev/null
    npm install
    log "Building Tauri GUI (release) — this can take 3-5 minutes…"
    npm run tauri build
    popd >/dev/null
    log "GUI binary: $REPO_DIR/jupiteros/src-tauri/target/release/jupiteros"
}
...
setup_amalthea_venv() {
    ...
    deactivate
    popd >/dev/null
    log "Amalthea venv ready: $REPO_DIR/moon-amalthea/.venv"
}
...
3. Launch the JupiterOS GUI:
     ./jupiteros/src-tauri/target/release/jupiteros

On first launch:
  - Moon Io's setup.rs auto-downloads Qdrant (~80 MB) and the BGE-M3 ONNX
    model (~570 MB) into moon-io-rs/data/ . Allow a few minutes.
