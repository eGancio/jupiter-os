...
fn europa_data_dir() -> PathBuf {
    ...
    let raw = if raw.is_empty() { "moons/europa/data".to_string() } else { raw };
...
/// DATA_DIR (`.../moons/europa/data`) whose parent is the crate root, so it
...
pub async fn whatsapp_start_pairing(state: State<'_, WhatsappPairing>) -> Result<(), String> {
    ...
    if !helper.exists() {
        return Err(format!(
            "Helper WhatsApp non trovato in {helper:?}. Esegui `npm ci` in moons/europa/whatsapp-helper."
        ));
    }
