...
export function AddMoonModal({ onClose, onAdded, suggestedPort }: Props) {
  ...
  return (
    ...
    >
      <div className="bg-jupiter-surface border border-jupiter-orange/30 rounded-xl shadow-2xl w-[440px] p-6 space-y-4">
        <h2 className="text-[13px] font-semibold text-white">{t("addMoon.title")}</h2>
          ...
          <div className="space-y-3">
            <label className="block space-y-1">
              ...
              <input autoFocus value={name} onChange={(e) => setName(e.target.value)}
                ...
                className="w-full bg-jupiter-elevated border border-white/10 rounded px-3 py-1.5 text-[12px] text-white placeholder:text-jupiter-dim outline-none focus:border-jupiter-orange/50" />
            </label>
            <label className="block space-y-1">
              <span className="text-[11px] text-jupiter-dim">{t("addMoon.command")}</span>
              <input value={command} onChange={(e) => setCommand(e.target.value)}
                placeholder={t("addMoon.commandPh")}
                className="w-full bg-jupiter-elevated border border-white/10 rounded px-3 py-1.5 text-[12px] text-white placeholder:text-jupiter-dim outline-none focus:border-jupiter-orange/50 font-mono" />
            </label>
            <div className="flex gap-3">
              ...
              <label className="w-20 block space-y-1">
                ...
                <input type="number" value={port} onChange={(e) => setPort(Number(e.target.value))}
                  className="w-full bg-jupiter-elevated border border-white/10 rounded px-3 py-1.5 text-[12px] text-white outline-none focus:border-jupiter-orange/50 font-mono" />
              </label>
            </div>
            <label className="block space-y-1">
              ...
              <textarea value={envText} onChange={(e) => setEnvText(e.target.value)} rows={3}
                ...
                className="w-full bg-jupiter-elevated border border-white/10 rounded px-3 py-1.5 text-[12px] text-white placeholder:text-jupiter-dim outline-none focus:border-jupiter-orange/50 font-mono resize-none" />
            </label>
          </div>
        ) : (
          <div className="space-y-3">
              ...
              <div className="flex gap-2">
                {(["http", "sse"] as const).map((t) => (
                  <button key={t} onClick={() => setRemoteType(t)}
                    className={`px-4 py-1.5 text-[12px] rounded border transition-colors font-mono ${
                      remoteType === t
                        ? "border-jupiter-orange text-jupiter-orange bg-jupiter-orange/10"
                        : "border-white/10 text-jupiter-dim hover:text-white"
                    }`}>
                    {t}
                  </button>
                ))}
        ...
        <div className="flex justify-end gap-2 pt-1">
          <button onClick={onClose}
            className="px-3 py-1.5 text-[12px] rounded text-jupiter-dim hover:text-white hover:bg-jupiter-elevated transition-colors">
            {t("common.cancel")}
          </button>
          <button onClick={handleConfirm} disabled={loading}
            className="px-4 py-1.5 text-[12px] rounded bg-jupiter-orange text-white hover:bg-jupiter-orange-light disabled:opacity-50 transition-colors">
