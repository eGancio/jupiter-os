...
//! of a company report. It lives in `moon-thebe/data/brands/<name>/` as a brand.json
...
/// not the repo root). Falls back to <base>/moon-thebe/data/brands.
fn brands_root() -> PathBuf {
    ...
    config::get_base_dir().join("moon-thebe/data/brands")
}
